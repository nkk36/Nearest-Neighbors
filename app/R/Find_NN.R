Find_NN = function(k = 5, company, minRev, maxRev, minAct, maxAct, includeAct = FALSE) {
  ...
  # Load data
  vendors = read.csv("data/knn_names.csv",
                     colClasses = "character")
  data = read.csv("data/knn_indices_values.csv")
  ...
  matrix = sparseMatrix(i = data$row, j = data$col, x = data$values)
  matrix = as.matrix(matrix)
  df = as.data.frame(matrix)
  
  
  df = cbind.data.frame(vendors, df)
